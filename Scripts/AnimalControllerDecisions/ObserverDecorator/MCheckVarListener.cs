...
namespace Malbers.Integration.AITree
{
    ...
    public class MCheckVarListener : ObserverDecorator
    {
        ...
        protected override void OnInitialize()
        {
            ...
            if (objectives != null && objectives.Length > 0)
            {
                foreach (var target in objectives)
                {
                    ...
                    {
                        case VarType.Bool:
                            decisionComponents.AddRange(GetComponents<BoolVarListener>(target.gameObject));
                            break;
                        case VarType.Int:
                            decisionComponents.AddRange(GetComponents<IntVarListener>(target.gameObject));
                            break;
                        case VarType.Float:
                            decisionComponents.AddRange(GetComponents<FloatVarListener>(target.gameObject));
                            break;
                    }
                }
            }
            ...
            foreach (var component in decisionComponents)
            {
                component.SendMessage("AddToList", SendMessageOptions.DontRequireReceiver); // Custom method to add to the list
            }
        }
        ...
        private VarListener[] GetListeners()
        {
            List<VarListener> result = new List<VarListener>();
            foreach (var component in decisionComponents)
            {
                if (component is VarListener)
                {
                    result.Add(component as VarListener);
                }
            }
