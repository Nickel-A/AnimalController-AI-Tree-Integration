...
namespace Malbers.Integration.AITree
{
    ...
    public class MLookForMalbersTags : ObserverDecorator
    {
        ...
        {
            return LookForMalbersTags(AIBrain);
        }

        public bool LookForMalbersTags(AIBrain AIBrain)
        {
                ...
                while (filteredTags.Count != 0)
                {
                    ...
                    if (go != null)
                    {
                        if (AIUtility.IsInFieldOfView(AIBrain, go.position, lookAngle, lookRange, lookMultiplier, obstacleLayer, out _))
                        {
                            if (assignTarget)
                            {
                                AIBrain.AIControl.SetTarget(go, moveToTarget);
                            }
                            return true;
                        }
                    }
            ...
            if (closest)
            {
                if (assignTarget)
                {
                    AIBrain.AIControl.SetTarget(closest.transform, moveToTarget);
                }
                return true;
            }
            return false;
